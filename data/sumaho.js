...
AFRAME.registerComponent('raycaster-listen', {
    ...
    this.el.addEventListener('raycaster-intersected', evt => {
      ...
      if(supportTouch)
      {
        maeX_sumaho = getObj.getAttribute('position').x;
        maeY_sumaho = getObj.getAttribute('position').y;
        maeZ_sumaho = getObj.getAttribute('position').z;

        zz_sumaho = maeZ_sumaho;
      }
  ...
  el.onclick = click_handler;
    ...
    function start_handler(ev) 
    {
      ...
      if (ev.targetTouches.length != 0) {
        ...
        for (let i = 0; i < ev.targetTouches.length; i++) {
          ...
          sumahoMaeX[i]= sumahoX[i];
          sumahoMaeY[i] = sumahoY[i];

          sumahoX[i] = tpCache[i].clientX;
          sumahoY[i] = tpCache[i].clientY;
        }
      } 
      else 
      {
    ...
    function move_handler(ev) 
    {
     ...
     let preAveY = 0;
      ...
      for(let i = 0; i < yubi; i++)
      {
        aveX = aveX + sumahoX[i];
        aveY = aveY + sumahoY[i];

        preAveX = preAveX + sumahoMaeX[i];
        preAveY = preAveY + sumahoMaeY[i];
      }
      if( yubi != 0)
      {
        aveX = aveX / yubi;
        ...
        preAveX = preAveX / yubi;
        preAveY = preAveY / yubi;
   ...
   /*
      ...
      */
      ...
      if(getObj != null && yubi != 0)
      {
        if(yubi == 1)
        {
          getObj.setAttribute('position', (maeX_sumaho + aveX) + ' ' + (maeY_sumaho + aveY) + ' ' + maeZ_sumaho);
        }
        if (pushSumaho && yubi == 2)
        {
          getObj.setAttribute('position', getObj.getAttribute('position').x + ' ' + getObj.getAttribute('position').y + ' ' + zz_sumaho);
        }
        maeX_sumaho = getObj.setAttribute('position').x;
        maeY_sumaho = getObj.setAttribute('position').y;
        maeZ_sumaho = getObj.setAttribute('position').z;
      }
    }
    ...
    function handle_pinch_zoom(ev) 
    {
      if (ev.targetTouches.length == 2) {
        ...
        if (point1 >= 0 && point2 >= 0) {
          ...
          zz_sumaho += (diff1 - diff2) * -0.003;
          // Restrict scale
        zz_sumaho = Math.min(Math.max(-7, zz_sumaho), -2);
