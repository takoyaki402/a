...
AFRAME.registerComponent('cursor-listener', {
	...
	tick: function () {
        ...
        {  
            ...
            if(changeMode == 0)
            {
                new_box_List[near[0]].setAttribute('position', intersection.x + ' ' + 0 + ' ' + (-intersection.y - 2));
                new_box_List[near[0]].setAttribute('rotation', cam.getAttribute('rotation'));
            }
            else if(changeMode == 1)
            {
                new_box_List[near[0]].firstElementChild.setAttribute('position', intersection.x + ' ' + 0 + ' ' + (-intersection.y - 2));
                new_box_List[near[0]].firstElementChild.setAttribute('rotation', cam.getAttribute('rotation'));
            }
            else
            {
                new_box_List[near[0]].lastElementChild.setAttribute('position', intersection.x + ' ' + 0 + ' ' + (-intersection.y - 2));
