...
function objPrefab()
{
  ...
  new_box_List.push(new_gltf_model);
  ...
  for(let i = 0; i < 5; i++)
  {
    ...
    new_gltf_model.setAttribute('dynamic-body', 'shape: cube; mass: 1');
    new_gltf_model.setAttribute('position', i + ' 0 -5');
    new_gltf_model.setAttribute('rotation', '0 0 0');
...
function move_handler(ev) {
  ...
  touch_pos = [ev.changedTouches[0].clientX, ev.changedTouches[0].clientY];
...
function prefab()
{
  ...
  if(now_box == 0)
  {
    ...
    //new_entity.setAttribute('color', 'yellow');
